...
<?php
/* This is an bot webhook, created by Felix Lepoutre. This webhook only reads what one or more specific user(s) in a group says, and then reposts that into a telegram channel. 
Instructions
...
//define vars
$superuser = <USERID>;
$allowedUsers = array("<USERID>"=>"","<USERID>"=>"<KEYWORD>","<USERID>"=>"<KEYWORD>","<USERID>"=>"<KEYWORD>"); //change <KEYWORD> for the string of characters the post must contain to be reposted. Works with #hashtags. The first user in this example array doesn't need a keyword, so everything will be reposted. 
$bottoken = "<TOKEN>";
...
foreach($allowedUsers as $x=>$xValue){
	if($userID == $x){
		if($xValue == "" || strpos($message, $xValue) !== false){
